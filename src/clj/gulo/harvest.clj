...
(defn harvest-all
  ...
  [local-path s3-bucket s3-path & {:keys [sync paths-file path-coll] :or {sync false path nil path-list nil}}]
  (if sync
    (sync-resource-table))
  (let [resource-urls (cond
                       paths-file (util/read-file paths-file)
                       path-coll path-coll
